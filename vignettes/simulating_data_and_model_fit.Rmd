...
onsets <- 90:100
cases_true_df <- df %>% 
  filter(time_onset==onset)
cases_df <- res$cases %>% 
  filter(time_onset==onset) %>% 
  mutate(cases_known=cases_true_df$cases_true[1]) %>% 
...
# MCMC on real data
Influenza-like illness in the US
```{r}
ILI_2014 %>% 
  mutate(time_delay=time_reported-time_onset) %>%
  ggplot(aes(x=time_onset, y=time_delay,
...
ILI_2014 %>% 
  group_by(time_onset) %>% 
  summarise(cases_reported1=cases_reported[1],
            cases_reported=sum(cases_reported, na.rm = T)) %>% 
  ggplot(aes(x=time_onset, y=cases_reported)) +
  geom_line() +
  geom_line(aes(y=cases_reported1))
```
Get the data above for a longer time series
using the DELPHI API

Measles NL 2013-2014
```{r}
measles_NL_2013 %>% 
  mutate(time_delay=time_reported-time_onset) %>%
  ggplot(aes(x=time_onset, y=time_delay,
...
snapshot_with_Rt_index_df <- snapshot_with_Rt_index_df %>% 
  ...
  mutate(time_onset=time_onset - df_sum$time_onset[1] + 1)
