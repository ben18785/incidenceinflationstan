...
```{r}
ILI_2014 <- read.csv("../../../../Downloads/ili_2014.csv") %>% 
  pivot_longer(-date_onset) %>% 
  mutate(time_delay=gsub("X", "", name)) %>% 
  mutate(time_delay=as.numeric(time_delay)) %>% 
  rename(cases_reported=value) %>%
  select(-name) %>% 
  mutate(date_onset=as.Date(date_onset,
                            format="%d/%m/%Y")) %>% 
  mutate(time_onset=as.numeric(date_onset-first(date_onset)) / 7) %>%
  mutate(time_reported=time_onset+time_delay) %>% 
  select(date_onset, time_onset, time_reported, cases_reported)
