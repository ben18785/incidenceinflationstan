...
Rt_prior <- list(location=2, scale=3)
...
initial_Rt <- tribble(~Rt_index, ~Rt,
                      ...
                      5, 1.0)
Rt_indices <- unlist(map(seq(1, 5, 1), ~rep(., 20)))
Rt_prior <- list(location=1.5, scale=2, is_gamma=TRUE)
Rt_index_lookup <- tibble(
  time_onset=seq_along(Rt_indices),
  Rt_index=Rt_indices)
...
stan_model <- cmdstanr::cmdstan_model("../src/stan/conditional_renewal_3.stan",
...
test <- mcmc_single(
  ...
  priors,
  s_params,
  initial_cases_true=res$cases %>% group_by(time_onset) %>% summarise(cases_true=last(cases_true)),
...
current_values <- list(
  ...
  reporting=list(location=10, scale=3)
)
