...
```{r}
max_cases <- 10000
df_est <- sample_cases_history(snapshot_with_Rt_index_df, max_cases, Rt_function, s_params, initial_reporting_parameters, maximise = F)
```
...
res$reporting %>% 
  ggplot(aes(x=iteration, y=mean, colour=as.factor(chain))) +
  geom_line() +
...
a <- sample_reporting(
      ...
      current_reporting_parameters=initial_reporting_parameters,
      prior_parameters=priors$reporting,
      metropolis_parameters=list(mean_step=0.25, sd_step=0.1),
      maximise=FALSE,
      ndraws=100)

a %>% 
  ggplot(aes(x=draw_index, y=mean)) +
  geom_line() +
  facet_wrap(~reporting_piece_index)

logp_current <- observation_process_all_times_logp(
    snapshot_with_true_cases_df=df_temp,
    reporting_parameters=initial_reporting_parameters %>% mutate(mean=c(10, 3))
  )
```
